...
public class calctemp {
	...
	public static class Map extends Mapper<LongWritable, Text, Text, FloatWritable> {
		public void map(LongWritable key, Text val, Context context) 
			...
			if(words[0].equals("USW00094728") && words[2].equals("TMAX")) {
				k = Integer.valueOf(words[3]);
			} else if(words[0].equals("USW00094728") && words[2].equals("TMIN")) {
				int temp = Integer.valueOf(words[3]);
				float ans = (float) (k - temp) / 10;
				context.write(new Text(words[1]), new FloatWritable(ans));
			}
		}
	}
